...
public final class TasteOld<D>: Model where D: QuerySupporting, D: IndexSupporting {
...
extension TasteOld: Migration where D: QuerySupporting, D: IndexSupporting { }
...
public struct TasteOldMigration<D>: Migration where D: QuerySupporting & SchemaSupporting & IndexSupporting {
  public typealias Database = D
  
  static func prepareFields(on connection: Database.Connection) -> Future<Void> {
    return Database.create(TasteOld<Database>.self, on: connection) { builder in
      
      //add fields
      try builder.field(for: \TasteOld<Database>.id)
      try builder.field(for: \TasteOld<Database>.name)
      try builder.field(for: \TasteOld<Database>.description)
      try builder.field(for: \TasteOld<Database>.actions)
      try builder.field(for: \TasteOld<Database>.sources)
  ...
  static func prepareInsertData(on connection: Database.Connection) ->  Future<Void>   {
    let futures : [EventLoopFuture<Void>] = tastes.map { name, desc, action, sources in
      return TasteOld<D>(name: name, description: desc, actions: action, sources: sources).create(on: connection).map(to: Void.self) { _ in return }
    }
